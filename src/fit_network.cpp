...
NumericMatrix Compute_logl_poisson_with_theta_FE_network(const List & X,  const NumericMatrix & Y, 
...
NumericMatrix Compute_logl_poisson_with_LR_FE_network(const List & X,  const NumericMatrix & Y, 
                                                      const NumericVector & beta, 
                                                      const NumericVector & fe_N, const NumericVector & fe_T,
...
List Compute_score_poisson_with_theta_FE_network(const List & X, const NumericMatrix Y, 
                                                const NumericVector & beta, 
                                                const NumericVector & fe_N, const NumericVector & fe_T,
                                                const NumericMatrix & Theta){
    const Map<MatrixXd> Y_ (as <Map<MatrixXd>> (Y));
...
List Compute_score_poisson_with_LR_FE_network(const List & X, const NumericMatrix Y, 
                                              const NumericVector & beta, 
                                              const NumericVector & fe_N, const NumericVector & fe_T,
                                              const NumericMatrix & L, const NumericMatrix & R){
...
double Compute_obj_poisson_with_theta_FE_network(const List & X,  const NumericMatrix & Y, 
                                                 const NumericVector & beta, 
                                                 const NumericVector & fe_N, const NumericVector & fe_T,
                                                 const NumericMatrix & Theta, 
                                                 const double sigma_sum, const double phi){
    NumericMatrix logl_poisson = Compute_logl_poisson_with_theta_FE_network( X,  Y, beta, fe_N, fe_T, Theta );
...
double Compute_obj_poisson_with_LR_FE_network(const List & X,  const NumericMatrix & Y, 
                                              const NumericVector & beta,
                                              const NumericVector & fe_N, const NumericVector & fe_T,
                                              const NumericMatrix & L, const NumericMatrix & R){
    NumericMatrix logl_poisson = Compute_logl_poisson_with_LR_FE_network( X,  Y, beta, fe_N, fe_T, L, R );
    Map<MatrixXd> logl_poisson_ (as <Map<MatrixXd>> (logl_poisson));
    double loss = logl_poisson_.sum();
    double obj  = -loss;
    return obj;
...
NumericMatrix Compute_first_order_poisson_with_LR_FE_network(const List & X, const NumericMatrix & Y,
                                                             const NumericVector & beta, 
                                                             const NumericVector & fe_N, const NumericVector & fe_T,
                                                             const NumericMatrix & L, const NumericMatrix & R){
    const Map<MatrixXd> Y_ (as <Map<MatrixXd>> (Y));
...
NumericMatrix Compute_second_order_poisson_with_LR_FE_network(const List & X,
                                                              const NumericVector & beta, 
                                                              const NumericVector & fe_N, const NumericVector & fe_T,
                                                              const NumericMatrix & L, const NumericMatrix & R){
    ...
    second_score_.diagonal().setZero();
...
NumericVector Update_beta_network(const NumericVector & beta, const NumericVector & score_beta, 
                                  const double s){
...
List Update_theta_network(const NumericMatrix & Theta, const NumericMatrix & score_theta,
...
List Update_LR_network(const NumericMatrix & L, const NumericMatrix & R, 
...
List Update_FE_network(const NumericVector & fe_N, const NumericVector & fe_T, 
...
List fit_poisson_FE_network(const List & X,  const NumericMatrix & Y, const double phi,  
    ...
    double obj_old = Compute_obj_poisson_with_theta_FE_network(X, Y, beta_old, 
                                                               fe_N_old, fe_T_old,
                                                               Theta_old, 
                                                               sigma_sum, phi);
    ...
    while(num_iter !=iter_max){
        double s_t = s;
        List list_score = Compute_score_poisson_with_theta_FE_network(X, Y, beta_old, fe_N_old, fe_T_old, Theta_old);
        ...
        while(1){
            beta_new = Update_beta_network(beta_old, score_beta, s_t/(Y_.size()));
            List list_fe = Update_FE_network(fe_N_old, fe_T_old, score_fe_N, score_fe_T, s_t/T, s_t/N);
            fe_N_new = as<NumericVector>(list_fe["fe_N_est"]);
            fe_T_new = as<NumericVector>(list_fe["fe_T_est"]);
            List list_theta = Update_theta_network(Theta_old, score_theta, s_t, phi);
            ...
            obj_new = Compute_obj_poisson_with_theta_FE_network(X, Y, beta_new, 
...
List MLE_poisson_FE_network(const List & X,  const NumericMatrix & Y, 
    ...
    double obj_old = Compute_obj_poisson_with_LR_FE_network(X, Y, beta_old, fe_N_old, fe_T_old, L_old, R_old);
    double obj_new = obj_old;
    ...
    while(num_iter !=iter_max){
        double s_t = s;
        List list_score = Compute_score_poisson_with_LR_FE_network(X, Y, beta_old, fe_N_old, fe_T_old, L_old, R_old);
        score_beta = as<NumericVector>(list_score["score_beta"]);
        ...
        while(1){
            beta_new = Update_beta_network(beta_old, score_beta, 2.0*s_t/(Y_.size()));
            List list_fe = Update_FE_network(fe_N_old, fe_T_old, score_fe_N, score_fe_T, s_t/T, s_t/N);
            fe_N_new = as<NumericVector>(list_fe["fe_N_est"]);
            fe_T_new = as<NumericVector>(list_fe["fe_T_est"]);
            List list_LR = Update_LR_network(L_old, R_old, score_L, score_R, 2.0*s_t/(Y_.cols()), 2.0*s_t/(Y_.rows()));
            L_new = as<NumericMatrix>(list_LR["L_est"]);
            R_new = as<NumericMatrix>(list_LR["R_est"]);
            //Rcout << L_old(0,0) << std::endl;
            //Rcout << L_new(0,0) << std::endl;
            
            obj_new = Compute_obj_poisson_with_LR_FE_network(X, Y, beta_new, fe_N_new, fe_T_new, L_new, R_new);
            //Rcout << obj_old<< std::endl;
            //Rcout << obj_new<< std::endl;
